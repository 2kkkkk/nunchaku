...
from .utils import NunchakukpModelLoaderMixin, pad_tensor
from ..._C import QuantizedFluxModel, utils as cutils
from ...lora.flux.nunchakukp_converter import fuse_vectors, to_nunchakukp
from ...lora.flux.utils import is_nunchakukp_format
...
class NunchakukpFluxTransformerBlocks(nn.Module):
    def __init__(self, m: QuantizedFluxModel, device: str | torch.device):
        super(NunchakukpFluxTransformerBlocks, self).__init__()
        self.m = m
...
class NunchakukpFluxTransformer2dModel(FluxTransformer2DModel, NunchakukpModelLoaderMixin):
    @register_to_config
    def __init__(
        self,
        patch_size: int = 1,
        ...
        super(NunchakukpFluxTransformer2dModel, self).__init__(
    ...
    def inject_quantized_module(self, m: QuantizedFluxModel, device: str | torch.device = "cuda"):
        ...
        self.transformer_blocks = nn.ModuleList([NunchakukpFluxTransformerBlocks(m, device)])
    ...
    def set_attention_impl(self, impl: str):
        ...
        assert isinstance(block, NunchakukpFluxTransformerBlocks)
    ...
    def update_lora_params(self, path_or_state_dict: str | dict[str, torch.Tensor]):
        ...
        if not is_nunchakukp_format(state_dict):
            state_dict = to_nunchakukp(state_dict, base_sd=self._quantized_part_sd)
    ...
    def forward(
        ...
        nunchakukp_block = self.transformer_blocks[0]
        encoder_hidden_states, hidden_states = nunchakukp_block(
