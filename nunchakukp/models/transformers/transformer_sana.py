...
from .utils import NunchakukpModelLoaderMixin
...
class NunchakukpSanaTransformerBlocks(nn.Module):
    def __init__(self, m: QuantizedSanaModel, dtype: torch.dtype, device: str | torch.device):
        super(NunchakukpSanaTransformerBlocks, self).__init__()
        self.m = m
        self.dtype = dtype
        self.device = device
    ...
    def forward(
        ...
        nunchakukp_encoder_hidden_states = encoder_hidden_states[mask > -9000]
        ...
        return (
            self.m.forward(
                ...
                nunchakukp_encoder_hidden_states.to(self.dtype).to(self.device),
...
class NunchakukpSanaTransformer2DModel(SanaTransformer2DModel, NunchakukpModelLoaderMixin):
    @classmethod
    ...
    def inject_quantized_module(self, m: QuantizedSanaModel, device: str | torch.device = "cuda"):
        self.transformer_blocks = torch.nn.ModuleList([NunchakukpSanaTransformerBlocks(m, self.dtype, device)])
        return self
...
def inject_quantized_module(
    ...
    net.transformer_blocks = torch.nn.ModuleList([NunchakukpSanaTransformerBlocks(m, net.dtype, device)])
    return net
