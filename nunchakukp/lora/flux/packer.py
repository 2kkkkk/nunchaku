...
class NunchakukpWeightPacker(MmaWeightPackerBase):
    ...
    def pack_weight(self, weight: torch.Tensor) -> torch.Tensor:
        ...
        # currently, Nunchakukp did not check the boundry of unrolled `k` dimension
        ...
        weight = weight.reshape(
            ...
            self.n_pack_size,  # always 2 in nunchakukp
            self.num_n_lanes,  # constant 8
            self.reg_n,  # constant 1
            k_tiles,
            self.num_k_packs,  # 1
            self.k_pack_size,  # always 2 in nunchakukp
    ...
    def pack_scale(self, scale: torch.Tensor, group_size: int) -> torch.Tensor:
        ...
        assert scale.dtype in (torch.float16, torch.bfloat16), "currently nunchakukp only supports fp16 and bf16."
        n = scale.shape[0]
        # nunchakukp load scales all in one access
