...
def get_sm_targets() -> list[str]:
    ...
    install_mode = os.getenv("NUNCHAKUKP_INSTALL_MODE", "FAST")
...
if __name__ == "__main__":
    fp = open("nunchakukp/__version__.py", "r").read()
    ...
    GCC_FLAGS = ["-DENABLE_BF16=1", "-DBUILD_NUNCHAKUKP=1", "-fvisibility=hidden", "-g", "-std=c++20", "-UNDEBUG", "-Og"]
    MSVC_FLAGS = ["/DENABLE_BF16=1", "/DBUILD_NUNCHAKUKP=1", "/std:c++20", "/UNDEBUG", "/Zc:__cplusplus", "/FS"]
    NVCC_FLAGS = [
        "-DENABLE_BF16=1",
        "-DBUILD_NUNCHAKUKP=1",
        "-g",
    ...
    if os.getenv("NUNCHAKUKP_BUILD_WHEELS", "0") == "0":
    ...
    nunchakukp_extension = CUDAExtension(
        name="nunchakukp._C",
        sources=[
            "nunchakukp/csrc/pybind.cpp",
    ...
    setuptools.setup(
        ...
        ext_modules=[nunchakukp_extension],
