...
def get_pipeline(
    ...
    if model_name == "schnell":
        if precision in ["int4", "fp4"]:
            ...
            if precision == "int4":
                transformer = NunchakukpFluxTransformer2dModel.from_pretrained("mit-han-lab/svdq-int4-flux.1-schnell")
            else:
                assert precision == "fp4"
                transformer = NunchakukpFluxTransformer2dModel.from_pretrained(
                    "mit-han-lab/svdq-fp4-flux.1-schnell", precision="fp4"
                )
            pipeline_init_kwargs["transformer"] = transformer
            if use_qencoder:
                from nunchakukp.models.text_encoders.t5_encoder import NunchakukpT5EncoderModel
        ...
        if precision == "int4":
            transformer = NunchakukpFluxTransformer2dModel.from_pretrained("mit-han-lab/svdq-int4-flux.1-dev")
